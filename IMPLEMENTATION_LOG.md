...
### Final batch verification and EXE packaging
- **Scope completed**: Finished the planned batch through Item 14, including the account/wallet model and migration hotfix, command palette fallback check, large dataset pagination/index pass, import/service tests, dialog migration confirmation, cash-flow forecast, budget variance explanations, recurring merchant detection, goal feasibility planner, category normal range bands, tax/accountant export, custom tax estimator, categorization rules engine upgrade, and local privacy/audit dashboard.
- **Final verification**: Ran `npm test`, `npm run typecheck`, `npm run build`, and `npm run dist` successfully on 2026-07-07. The final test run passed 5 test files and 23 tests. The production build completed with the existing Vite dynamic/static import warning for `database-encrypted.ts`; no build failure occurred.
- **EXE output**: `npm run dist` produced the Windows installer at `C:\Users\troll\Desktop\Budget\release\Budget Setup 1.0.0.exe` and the unpacked app executable at `C:\Users\troll\Desktop\Budget\release\win-unpacked\Budget.exe`.
- **Remaining verification limit**: Automated and executable verification passed, including targeted migration/math/data scenarios from each item. Full live click-through against a disposable unlocked Electron profile remains the only unperformed check because Electron app-data isolation on this Windows host continued resolving toward the real encrypted profile; I avoided unlocking or mutating the real user profile during verification.

### Follow-up: Account starting balances and savings-account balance behavior
- **Problem**: Account balances were computed only from transactions, so a real bank account that already had money before app history began appeared as zero until transactions accumulated. Savings transactions also needed clearer account-tab behavior for savings-type accounts.
- **Implementation**: Added additive `accounts.opening_balance` migration support in encrypted and legacy schema paths. Account create/update now accepts a starting balance, and `accounts:list` returns `opening_balance`, transaction activity balance, income/expense/savings activity totals, and final balance. Balance is now `opening_balance + account activity`; savings transactions subtract from checking/cash/other accounts but add to savings-type accounts when assigned there. The Accounts page now has total/everyday/savings summary cards, a starting-balance field in add/edit, and per-account balance breakdowns.
- **Verification**: `npm run typecheck`, `npm test`, and `npm run build` pass. Ran a focused old-schema SQLite migration check confirming `opening_balance` is added with default `0`. Ran account math checks confirming a checking account with `1000` opening balance, `3000` income, `900` expense, and `400` savings outflow computes to `2700`, while a savings account with `5000` opening balance, `700` savings activity, and `100` transfer out computes to `5600`.

### Follow-up: Expected yearly tax owed reconciliation
- **Problem**: The Tax estimator showed the raw year-to-date sum of monthly over/underpayment differences, but users who already know their expected yearly tax liability needed a way to reconcile that raw running total against the real yearly amount without changing any monthly entries.
- **Implementation**: Added persistent `tax_year_settings` storage in encrypted and legacy schema paths, keyed by tax year with nullable `expected_yearly_tax_owed`. Added `tax:getYearSettings` and `tax:setYearSettings` IPC/preload APIs. The Tax page now shows an optional "Expected Yearly Tax Owed" section beside the YTD total, with visible explanatory copy that the full amount is subtracted immediately and not prorated. When the field is blank, the original "Year to date total" display and raw monthly-sum behavior are preserved. When the field contains a number, the page shows both "Year-to-date (before reconciliation)" and "Year-to-date (reconciled)", using the exact formula `monthly difference sum - expected yearly tax owed`. Monthly gross/net/supposed values and per-month result calculations are not modified by reconciliation.
- **Verification**: `npm run typecheck`, `npm test -- --run`, and `npm run build` pass. Added focused renderer tests for the required math cases: `500 - 12000 = -11500`, changing the field recomputes from raw `500 - 9000 = -8500` without stacking, `14000 - 12000 = 2000`, and blank input returns the raw monthly sum. Ran an executable display-value check using the app's Swedish money formatting: Example A raw `+500,00 kr`, Example A reconciled `-11 500,00 kr`, edited expected tax `-8 500,00 kr`, Example B raw `+14 000,00 kr`, Example B reconciled `+2 000,00 kr`, cleared field raw `+500,00 kr`; the month-level differences stayed `+500,00 kr`, `0,00 kr`, `0,00 kr`. Live restart verification against the real encrypted profile was not run because it would require unlocking/mutating the user's active local financial database; persistence is covered by the new database table plus IPC round-trip wiring and build/type/test checks.
